...
| Pack | Author | License | Used for |
|---|---|---|---|
| [KayKit Character Pack: Adventurers](https://github.com/KayKit-Game-Assets/KayKit-Character-Pack-Adventures-1.0) | Kay Lousberg (kaylousberg.com) | CC0 | Animated hero models: Knight, Mage, Rogue (Ranger) with idle/run/attack/death animations; Barbarian (Torvald) + unhooded Rogue (Barlow) as two of the six townsfolk-only bodies so no NPC mirrors a player hero |
| [RPG Characters](https://quaternius.com) | Quaternius | CC0 | Four more townsfolk-only bodies (`public/models/TownCleric.gltf`, `TownMonk.gltf`, `TownScout.gltf`, `TownDrifter.gltf`, from the pack's Cleric/Monk/Ranger/Rogue), each with its own idle animation - so the tavern's remaining four townsfolk (Maribel, Zoltan, and both patrons) each get a body distinct from Torvald/Barlow and from each other, instead of repeating the same one or two KayKit silhouettes |
