...
export function buildNpcModel(classId, name, opts = {}) {
  ...
  // mage wand in Rosalind's hand, the knight sword, ...) AND the adventurer
  // cape (the user flagged Magda's - barkeeps don't wear capes). Held props
  // like the patrons' mugs are separate meshes added afterwards, unaffected.
  mesh.traverse((o) => {
    if (o.isMesh && /sword|mace|axe|bow|crossbow|wand|staff|hammer|dagger|blade|weapon|shield|spear|cape/i.test(o.name || '')) o.visible = false;
  });
...
export function buildDungeonMeshes(dungeon, theme, floor = 1) {
  ...
  if (dungeon.town) {
    ...
    for (const v of dungeon.vendors) {
      ...
      const VENDOR_NPC = {
        // Vendors use the townsfolk-only bodies so they can never mirror the
        // player's own hero AND so no two townsfolk share a silhouette: each
        // of the 6 town NPCs (3 vendors, Barlow, 2 patrons) gets one of 6
...
function buildTownDecor(group, dungeon, smokePuffs, townGlows = [], breakables = []) {
  ...
  if (dungeon.tavern) {
    ...
    function buildFacade(halfW, frontZ, sideSpan, scale = 1, doorScale = scale) {
      ...
      // sideInset, frontZ itself) are left untouched: only each element's
      ...
      const mkWindow = (x, z, roty) => {
        ...
        const top = new THREE.Mesh(new THREE.BoxGeometry(0.6 * S, 0.06 * S, 0.05 * S), timber); top.position.y = 0.26 * S;
        const bot = top.clone(); bot.position.y = -0.26 * S;
        const l = new THREE.Mesh(new THREE.BoxGeometry(0.06 * S, 0.58 * S, 0.05 * S), timber); l.position.x = -0.27 * S;
        const r = l.clone(); r.position.x = 0.27 * S;
        frame.add(top, bot, l, r);
        frame.position.copy(win.position);
        frame.rotation.y = roty;
        // mullions: a cross dividing the pane into four small lights
        const mV = new THREE.Mesh(new THREE.BoxGeometry(0.03 * S, 0.46 * S, 0.03 * S), timber);
        const mH = new THREE.Mesh(new THREE.BoxGeometry(0.5 * S, 0.03 * S, 0.03 * S), timber);
      ...
      // doorH/doorW are the overlay's OWN fixed design size, scaled by S: at
      // S=1 the door read barely taller than the 1.6-unit hero (user report,
      // TODO 692) because these were flat constants that never grew with the
      // model's node.scale bump. Since the mask fully covers the model's own
